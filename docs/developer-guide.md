...
refused (`readings_rejected`, from `modernize.py`; the line keeps its
original text and is not sent back on its own — the service is
deterministic and would answer the same), an entity that reached
